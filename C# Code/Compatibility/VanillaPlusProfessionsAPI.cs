    ...
    public class VanillaPlusProfessionsAPI : IVanillaPlusProfessions
    {
        ...
        public List<int> GetNextTierProfessions(Farmer who, int level, string skill)
        {
            ...
            if (level == 15)
            {
                foreach (var item in ModEntry.Professions)
                {
                    if (Game1.player.professions.Contains(item.Value.Requires) && DisplayHandler.CoreDisplayHandler.Value.AreSkillConditionsMet(skill, -1) == item.Value.Skill.ToString())
            ...
            else if (level == 20)
            {
                foreach (var item in ModEntry.Professions)
                {
                    if (level == item.Value.LevelRequirement && DisplayHandler.CoreDisplayHandler.Value.AreSkillConditionsMet(skill, -1) == item.Value.Skill.ToString())
                        toReturn.Add(item.Value.ID);
                }
            }
            return toReturn;
        ...
        {
            b.Draw(DisplayHandler.CoreDisplayHandler.Value.ProfessionIcons, position, new((profession - 467830) % 6 * 16, (profession - 467830) / 6 * 16, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
        }
