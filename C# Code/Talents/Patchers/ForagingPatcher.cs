            ...
            if (__result && !__instance.falling.Value && __instance.stump.Value && Game1.random.NextBool(0.5))
            {
                if (TalentUtility.AnyPlayerHasTalent("NatureSecrets"))
                {
                    List<string> strings = (from forageData in __instance.Location.GetData().Forage
                                           where GameStateQuery.CheckConditions(forageData.Condition ?? "TRUE", __instance.Location ?? t.getLastFarmerToUse().currentLocation, t.getLastFarmerToUse())
                    ...
                    if (strings.Count > 0)
                    {
                        ...
                        while (!success)
                        {
                            ...
                            if (!ItemContextTagManager.HasBaseTag(str, TalentCore.ContextTag_Banned_NatureSecrets) && ItemRegistry.Exists("(O)" + str))
                            {
                                Game1.createObjectDebris(str, (int)__instance.Tile.X, (int)__instance.Tile.Y, __instance.Location);
                                success = true;
                            }
                        }
                    }
